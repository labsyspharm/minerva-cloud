...
db_host = ssm.get_parameter(Name="/{}/{}/common/DBHost".format(STACK_PREFIX, STAGE))[
    "Parameter"
]["Value"]

db_port = ssm.get_parameter(Name="/{}/{}/common/DBPort".format(STACK_PREFIX, STAGE))[
    "Parameter"
]["Value"]

db_user = ssm.get_parameter(Name="/{}/{}/common/DBUser".format(STACK_PREFIX, STAGE))[
    "Parameter"
]["Value"]
...
db_name = ssm.get_parameter(Name="/{}/{}/common/DBName".format(STACK_PREFIX, STAGE))[
    "Parameter"
]["Value"]
...
def add_s3_manifest_keys_to_import(event, context):
    ...
    keys = [item.key.strip(prefix) for item in bucket.objects.filter(Prefix=prefix)]
...
def prepare_environment(event, context):
    ...
    try:
        ...
        job_definition = ssm.get_parameter(
            Name="/{}/{}/batch/SyncJobDefinitionARN".format(STACK_PREFIX, STAGE)
        )["Parameter"]["Value"]

        # Get parameters
        bucket = raw_bucket.split(":")[-1]
        import_uuid = event["import_uuid"]
        batch_parameters = {"s3uri": f"s3://{bucket}/{import_uuid}", "dir": import_uuid}
...
def start_scan_sfn(event, context):
    ...
    try:
        ...
        response = sfn.start_execution(
            stateMachineArn=scan_sfn, input=json.dumps({"import_uuid": import_uuid})
        )
