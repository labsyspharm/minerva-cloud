...
def make_response(
    code: int, body: Union[Dict, List], content_type="application/json"
) -> Dict[str, Any]:
    """Build a response.
...
def response(
    code: int, content_type="application/json"
...
def _validate_name(s):
    if len(s) > _length_name or _valid_name.match(s) is None:
        raise ValueError(
            "Name is invalid. Valid names begin with a letter, "
...
def _validate_image_name(s):
    if len(s) > _length_name or _valid_file_name.match(s) is None:
        raise ValueError(
            "Filename is invalid. Valid names contain only alphanumeric "
...
class Handler:
    ...
    def _has_permission(
        self, user: str, resource_type: str, resource: str, permission: str
    ):
        ...
        """
        if not self.client.has_permission(user, resource_type, resource, permission):
            raise AuthError("Permission Denied")

    def _is_member(
        self, group: str, user: str, membership_type: Optional[str] = "Member"
    ):
    ...
    def cognito_details(self, event, context):
        self.count += 1
        return {"eventRequestContext": event["requestContext"], "count": self.count}
    ...
    def create_repository(self, event, context):
        ...
        return self.client.create_repository(uuid, name, self.user_uuid, raw_storage)
    ...
    def create_image(self, event, context):
        ...
        uuid = str(uuid4())
        return self.client.create_image(
            uuid,
            name,
            pyramid_levels,
            format,
    ...
    def create_import(self, event, context):
        ...
        self._has_permission(self.user_uuid, "Repository", repository_uuid, "Write")
        from uuid import uuid4
    ...
    def create_membership(self, event, context):
        ...
        return self.client.create_membership(group_uuid, user_uuid, membership_type)
    ...
    def create_metadata(self, event, context):
        ...
        s3.put_object(
            Bucket=bucket, Key=image_uuid + "/metadata.xml", Body=ET.tostring(e_root)
        )
    ...
    def get_import_credentials(self, event, context):
        ...
        if self.client.get_import(uuid)["data"]["complete"] is not False:
            raise AuthError(f"Import is complete and can not be written: {uuid}")
    ...
    def update_import(self, event, context):
        ...
        if complete is True and import_["data"]["complete"] is True:
            ...
            sfn.start_execution(
                stateMachineArn=self.sync_sfn, input=json.dumps({"import_uuid": uuid})
            )
    ...
    def get_image_metadata(self, event, context):
        ...
        obj = boto3.resource("s3").Object(bucket, f"{bucket_key}/metadata.xml")
        body = obj.get()["Body"]
        data = body.read()
        return data
    ...
    def update_repository(self, event, context):
        ...
        if "access" in self.body:
            ...
            if access == "PublicRead":
                self.client.grant_repository_to_subject(
                    uuid, public_read_group["uuid"], "Read"
                )
            elif access == "Private":
    ...
    def update_membership(self, event, context):
        ...
        if membership_type is not None:
            self._is_member(group_uuid, self.user_uuid, "Owner")
        return self.client.update_membership(group_uuid, user_uuid, membership_type)
        ...
        for permission in permissions:
            if resource_type.lower() == "repository":
                self._has_permission(
                    self.user_uuid, "Repository", resource_uuid, "Admin"
                )
                self.client.grant_repository_to_subject(
                    resource_uuid, grantee, permission
                )
            else:
                raise ValueError(
                    "Grant not implemented yet for resource ", resource_type
                )
